...
##fitting "naive" model (plus period effects from panel unit root test conclusion) and checking for temporal dependence
###model 1
borrowing<-plm(total~lag_total+conditions+lag_cvote+log_gdp+polity,
               data,effect="time",model="pooling",index=c("country","year"))
bptest(borrowing) #heteroskedasticity
#moran.test(borrowing$residuals,)
pbgtest(borrowing) #no serial correlation at 0.05 significance level
plmtest(borrowing, effect = "individual", type = c("bp")) #pooling ok
plm:::summary.plm(borrowing, #using country clustered se
                  vcov=vcovHC(borrowing,method=c("white1"),type=c("HC1"),cluster=c("group"))) 
###model 2
conflict<-plm(conflicts~conditions+log_gdp+polity,
              data,effect="time",model="pooling",index=c("country","year"))
bptest(conflict) #heteroskedasticity
bgtest(conflict) #serial correlation
conflict<-{#trying LDV
  data %>%
    ungroup() %>%
    mutate(lag_conflicts=dplyr::lag(conflicts)) %>%
    plm(data=.,formula=conflicts~lag_conflicts+conditions+log_gdp+polity,
...
#fgls?
###model 3
emissions<-plm(emissions~log_cloan+log_gdp+polity,
               data,effect="time",model="pooling",index=c("country","year"))
bptest(emissions) #heteroskedasticity
bgtest(emissions) #serial correlation
...
Fixed effects (if no FE, we're good with robust, if FE, arrellano-bond)
Heteroskedasticity (done without FE;country clustered)
