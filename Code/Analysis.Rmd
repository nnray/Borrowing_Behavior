...
library(spatialreg);library(tidyverse)
```
...
###model 2
#will need lagsarlm instead of plm
conflict<-plm(conflicts~lag_conflicts+conditions+log_gdp+polity,
              data,effect="time",model="pooling",index=c("country","year"))
bptest(conflict) #heteroskedasticity
bgtest(conflict) #serial correlation
###model 3
emissions<-plm(emissions~lag_emissions+log_cloan+lag_log_cloan+log_gdp,
               data,effect="time",model="pooling",index=c("country","year"))
