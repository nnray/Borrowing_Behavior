...
a<-{
  data %>%
  select(country,year,total,lag_total,conflicts,emissions,conditions,cloan,log_cloan,gdp,log_gdp,polity,lag_cvote) %>%
  as.data.frame() %>%
  amelia(.,m=5,ts="year",cs="country")
}
all_imputations<-bind_rows(unclass(a$imputations),.id ="m") %>%
  group_by(m) %>%
  nest()
...
melded_summary #model 1 from imputed data
################################################################################
...
#GWPR.light::GWPR.moran.test(model1, w) if balanced
###model 2
model2<-lagsarlm(conflicts~lag_conflicts+conditions+log_gdp+polity+as.factor(year),
                 data=data,w,Durbin=~log_gdp+polity,zero.policy = TRUE)
...
model3<-lagsarlm(emissions~lag_emissions+log_cloan+lag_log_cloan+log_gdp+as.factor(year),
                 data=data,w,Durbin=emissions~log_gdp,zero.policy = TRUE)
