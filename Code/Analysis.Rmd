...
library(stargazer);library(readstata13);library(tidyverse)
```
...
#figure 1
figure1.1_data<-panel_data(data,id=country,wave=year) %>% #to use with line_plot()
  subset(country %in% c("Algeria","Angola","Benin","Botswana",
                                        ...
                                        "Kenya","Lesotho"))
figure1.1<-figure1.1_data %>% #using 1700 width, 1200 height for export
  line_plot(log_wbloan,
            overlay = FALSE) +
  geom_line(mapping=aes(y=log_cloan),data=figure1.1_data)
figure1.2_data<-panel_data(data,id=country,wave=year) %>%
  subset(country %in% c("Liberia","Libya",
...
figure1.2<-figure1.2_data %>% #using 1700 width, 1200 height for export
  line_plot(log_wbloan,
            overlay = FALSE) +
  geom_line(mapping=aes(y=log_cloan),data=figure1.2_data)
################################################################################
#figure 2
figure2.1<-data %>%
  drop_na() %>%
...
#plot.ts() #use console and set width 550 for these, height 500 for export
figure2.2<-data %>%
  group_by(year) %>%
  summarise(log_cloan=sum(log_cloan)) %>%
  select(-year) %>%
  ts(.,start = 2004,end = 2017)
figure2.3<-data %>%
  group_by(year) %>%
  summarise(log_wbloan=sum(log_wbloan)) %>%
  select(-year) %>%
  ts(.,start = 2004,end = 2017)
```
