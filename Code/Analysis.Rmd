...
library(stargazer);library(readstata13);library(spdep);library(lmtest);library(tidyverse)
```
...
################################################################################
#spatial/temporal dependence diagnostics
##w matrix
#w<-data %>%
#  select(country,log_cloan,log_wbloan) %>%
#  merge(africa,.,by=c("country")) %>%
#  st_queen() %>%
#  st_as_nb()
##fitting "naive" model (plus period effects from panel unit root test conclusion) and checking for temporal/spatial dependence
###model 1
borrowing<-plm(total~conditions+lag_cvote+log_gdp+polity,data,effect="time",model="pooling",index=c("country","year")) %>% summary()
#moran.test(borrowing$residuals,)
bgtest(borrowing) #some evidence of serial correlation, will try a LDV
borrowing<-{
  data %>%
    ungroup() %>%
    mutate(lag_total=dplyr::lag(total)) %>%
    plm(data=.,formula=total~lag_total+conditions+lag_cvote+log_gdp+polity,effect="time",model="pooling",index=c("country","year")) %>%
    summary()
}
bgtest(borrowing) #serial correlation increased...
###model 2
conflict<-plm(conflicts~log_cloan+log_gdp+polity,data,effect="time",model="pooling",index=c("country","year")) %>% summary()
###model 3
emissions<-plm(emissions~conditions+log_gdp,data,effect="time",model="pooling",index=c("country","year")) %>% summary()
