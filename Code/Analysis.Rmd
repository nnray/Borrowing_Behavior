...
summary<- summary %>%
  ...
  summarise(`Mean Debt as Percentage of GDP`=mean(debt),
            `Max Debt as Percentage of GDP`=max(debt),
            `Total Logged Loans from China`=sum(log_cloan),
            `Total Logged Loans from World Bank`=sum(log_wbloan))
png("summary1.png", height = 22*nrow(summary), width = 210*ncol(summary))
grid.table(summary)
dev.off()
```
```{r summary2}
summary2<- data %>%
  group_by(country) %>%
  summarise(`Mean GDP per capita`=mean(gdp),
            `SD GDP per capita`=sd(gdp),
            `Mean Logged Chinese Loans`=mean(log_cloan),
            `SD Logged Chinese Loans`=sd(log_cloan),
            `Mean Logged World Bank Loans`=mean(log_wbloan),
            `SD Logged World Bank Loans`=sd(log_wbloan),
            `Mean Polity`=mean(polity),
            `SD Polity`=sd(polity),
