...
library(spatialreg);library(Amelia);library(broom);library(tidyverse)
```
...
w_imputed<-read.gal(here("w_imputed"))
################################################################################
...
#spatial dependence in residuals?
################################################################################
#models with imputed data; code heavily borrowed from
#https://www.andrewheiss.com/blog/2018/03/07/amelia-tidy-melding/
##model 1
all_imputations<-bind_rows(unclass(amelia$imputations),.id ="m") %>%
  group_by(m) %>%
  nest()
...
model_degree_freedom <- models_imputations %>%
  ...
  filter(m == "imp1") %>%
  pull(df.residual)
melded_summary1 <- as.data.frame(cbind(t(coefs_melded$q.mi),
...
##model 2
w_imputed<-nb2listw(w_imputed,style="W",zero.policy = TRUE) #for model 2 and 3
models_imputations <- all_imputations %>%
...
just_ses <- params %>%
  ...
  select(-m, -key)
coefs_melded <- mi.meld(just_coefs, just_ses)
model_degree_freedom <- nrow(data)-length(just_coefs)
melded_summary2 <- as.data.frame(cbind(t(coefs_melded$q.mi),
                                      t(coefs_melded$se.mi))) %>%
  magrittr::set_colnames(c("estimate", "std.error")) %>%
  mutate(term = rownames(.)) %>%
  select(term, everything()) %>%
...
just_ses <- params %>%
  ...
  select(-m, -key)
coefs_melded <- mi.meld(just_coefs, just_ses)
model_degree_freedom <- model_degree_freedom <- nrow(data)-length(just_coefs)
melded_summary3 <- as.data.frame(cbind(t(coefs_melded$q.mi),
                                      t(coefs_melded$se.mi))) %>%
  magrittr::set_colnames(c("estimate", "std.error")) %>%
  mutate(term = rownames(.)) %>%
  select(term, everything()) %>%
