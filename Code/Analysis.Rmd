...
model2<-plm(conflicts~lag_conflicts+conditions+log_gdp+polity,
              data,effect="time",model="pooling",index=c("country","year"))
bptest(model2) #heteroskedasticity
pbgtest(model2) #serial correlation
###model 3
model3<-plm(emissions~lag_emissions+log_cloan+lag_log_cloan+log_gdp,
               data,effect="time",model="pooling",index=c("country","year"))
bptest(model3) #heteroskedasticity
bgtest(model3) #serial correlation
################################################################################
