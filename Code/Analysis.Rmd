...
library(stargazer);library(panelr); library(tidyverse)
```
...
#conditions (have to include condition data too)
data2<-data %>%
  drop_na() %>%
  group_by(year) %>%
  summarise(conditions=sum(avg_conditions),conflict=sum(conflict)) %>% #sum of every country's avg conditions per project in a given year
  select(-year)
data_ts<-data2 %>%
