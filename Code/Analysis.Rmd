...
figure2.2<-china %>% #using full sample of Chinese data (2000-2017), nominal dollars
  drop_na %>%
  group_by(year) %>%
  summarise(cloan=sum(cloan)) %>%
  select(-year) %>%
...
figure2.3<-data %>%
  ...
  select(-year) %>%
  ts(.,start = 2004,end = 2017)
#type "plot.ts(figure2._)" in console, use width 550 and height 500 for export
```
